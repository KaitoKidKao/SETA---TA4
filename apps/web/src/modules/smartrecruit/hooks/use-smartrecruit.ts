...
export function useSkillGaps(jobTitle: string, criteriaId?: string | null) {
  return useQuery({
    queryKey: ['smartrecruit', 'skill-gaps', jobTitle, criteriaId ?? null],
    queryFn: () => smartrecruitApi.getSkillGaps(jobTitle, criteriaId),
    enabled: !!jobTitle,
  });
}
