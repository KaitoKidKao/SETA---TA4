...
Sign in as any CSV user or the admin `canh.ta@setafutureorg.onmicrosoft.com` with password `ChangeMe@2026`.

Useful flags:

- `--tenant <slug>` — bootstrap a differently-named tenant (default `setafutureorg`)
- `--admin-email <email>` — different admin (default `canh.ta@setafutureorg.onmicrosoft.com`)
...
> Bootstrap my local dev environment. Assume Docker, Node 24, and pnpm 9 are installed and `.env` is populated. Run `pnpm install`, `pnpm db:up`, `pnpm db:migrate`, then `pnpm db:seed` to load the SETA Future Org demo data. Verify by starting `pnpm dev` and reporting whether <http://localhost:5173/login> accepts `canh.ta@setafutureorg.onmicrosoft.com` / `ChangeMe@2026`. Stop and ask before running anything destructive.
