...
| **Nguyễn Đức Cường** | Backend Developer | Xây dựng backend module bằng Node 24, Hono và Mastra Workflow. Quản lý luồng file, Drizzle ORM và Postgres/pgvector cho Memory. |
| **Lê Minh Tuấn** | Frontend & UI/UX Developer | Xây dựng Dashboard bằng React 19, TanStack Router và Vite. Thiết kế giao diện luồng duyệt của con người (Human-in-the-loop Gates). |
...
*   **Quản lý Bộ nhớ (Memory):** Bộ nhớ ngắn hạn (Short-term) lưu lịch sử chat; Bộ nhớ làm việc (Working Memory) quản lý tạm thời tiêu chí tuyển dụng (`DS-07`), danh sách ứng viên (`DS-06`) và templates (`DS-08`); Bộ nhớ dài hạn (Long-term) lưu trữ Vector Embeddings của các hồ sơ cũ trong Postgres/pgvector để phục vụ cho tìm kiếm ngữ nghĩa sâu.
...
| **Chạm hạn mức API LLM** | Backend bắt mã lỗi 429 hoặc lỗi Timeout từ nhà cung cấp API. | **Retry with Jitter:** Kích hoạt graphile-worker kết hợp transactional outbox, áp dụng exponential backoff kết hợp jitter gửi lại yêu cầu sau khoảng thời gian tăng dần. | Nhìn thấy trạng thái "Đang xử lý trong hàng đợi" trên UI thay vì crash hệ thống. |
...
*   **Giao diện người dùng (User Interface):** SPA bằng React 19, TanStack Router và Vite hiển thị khu vực tải file (JD & CVs), bảng xếp hạng ứng viên, bảng điểm scorecard chi tiết, và cửa sổ soạn thảo email tích hợp nút gửi.
*   **Backend & Workflow Logic:** Node 24, Hono và Mastra Workflow xử lý các luồng file và tác vụ tuyển dụng. Dữ liệu được quản lý trong module schema `smartrecruit` bằng Drizzle ORM theo chuẩn module hóa của nền tảng Seta.
*   **LLM Provider & Model:** Lựa chọn model qua model registry và biến môi trường `AGENT_MODELS`; fallback hiện tại là **OpenAI GPT-4o-mini** nhờ tốc độ phản hồi nhanh, tính nhất quán trong cấu trúc JSON trả về và chi phí API tối ưu.
*   **Cơ chế lưu trữ và bộ nhớ (Memory Architecture):**
    *   *Short-term Memory:* Bộ nhớ phiên và thread metadata của nền tảng Agent lưu trạng thái làm việc của nhà tuyển dụng.
    *   *Working Memory:* Quản lý trực tiếp cấu trúc dữ liệu tiêu chí sàng lọc và nội dung CV đang so khớp.
    *   *Long-term Memory / Retrieval:* Postgres/pgvector lưu trữ các đoạn văn bản (chunks) của CV cũ đã số hóa dưới dạng Vector Embeddings phục vụ tính năng tìm kiếm ngữ nghĩa tái tương tác.
...
| **Sự không ổn định của LLM** | Khi tải lên quá nhiều hồ sơ cùng lúc, thời gian phản hồi có thể bị lâu hoặc vượt hạn mức chi phí API (API rate limit). | Áp dụng cơ chế xếp hàng tác vụ bằng graphile-worker và transactional outbox để xử lý hồ sơ không đồng bộ theo lô (batch processing), đồng thời tối ưu hóa kích thước prompt để kiểm soát lượng input token tối thiểu mà không ảnh hưởng tới chất lượng lập luận. |
