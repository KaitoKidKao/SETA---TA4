...
| **Nguyễn Đức Cường** | Backend Developer | Xây dựng API Server bằng Python FastAPI. Quản lý luồng file và tích hợp Vector DB (ChromaDB) cho Memory. |
| **Lê Minh Tuấn** | Frontend & UI/UX Developer | Xây dựng Dashboard bằng Next.js. Thiết kế giao diện luồng duyệt của con người (Human-in-the-loop Gates). |
...
*   **Quản lý Bộ nhớ (Memory):** Bộ nhớ ngắn hạn (Short-term) lưu lịch sử chat; Bộ nhớ làm việc (Working Memory) quản lý tạm thời tiêu chí tuyển dụng (`DS-07`), danh sách ứng viên (`DS-06`) và templates (`DS-08`); Bộ nhớ dài hạn (Long-term) lưu trữ Vector Embeddings của các hồ sơ cũ trong Vector Database (ChromaDB) để phục vụ cho tìm kiếm ngữ nghĩa sâu.
...
| **Chạm hạn mức API LLM** | Backend bắt mã lỗi 429 hoặc lỗi Timeout từ nhà cung cấp API. | **Retry with Jitter:** Kích hoạt hàng đợi Celery, áp dụng exponential backoff kết hợp jitter gửi lại yêu cầu sau khoảng thời gian tăng dần. | Nhìn thấy trạng thái "Đang xử lý trong hàng đợi" trên UI thay vì crash hệ thống. |
...
*   **Giao diện người dùng (User Interface):** SPA bằng Next.js hiển thị khu vực tải file (JD & CVs), bảng xếp hạng ứng viên, bảng điểm scorecard chi tiết, và cửa sổ soạn thảo email tích hợp nút gửi.
*   **Backend & Workflow Logic:** Python FastAPI xử lý bất đồng bộ các luồng file dung lượng lớn. Sử dụng cấu trúc module hóa rõ ràng theo chuẩn kỹ thuật của ban tổ chức.
*   **LLM Provider & Model:** Lựa chọn **OpenAI GPT-4o-mini** (phối hợp GPT-4o cho tác vụ cần lập luận phức tạp) nhờ tốc độ phản hồi nhanh, tính nhất quán trong cấu trúc JSON trả về và chi phí API tối ưu.
*   **Cơ chế lưu trữ và bộ nhớ (Memory Architecture):**
    *   *Short-term Memory:* Redis/Memory Cache lưu trạng thái phiên làm việc của nhà tuyển dụng.
    *   *Working Memory:* Quản lý trực tiếp cấu trúc dữ liệu tiêu chí sàng lọc và nội dung CV đang so khớp.
    *   *Long-term Memory / Retrieval:* CSDL Vector ChromaDB lưu trữ các đoạn văn bản (chunks) của CV cũ đã số hóa dưới dạng Vector Embeddings phục vụ tính năng tìm kiếm ngữ nghĩa tái tương tác.
...
| **Sự không ổn định của LLM** | Khi tải lên quá nhiều hồ sơ cùng lúc, thời gian phản hồi có thể bị lâu hoặc vượt hạn mức chi phí API (API rate limit). | Áp dụng cơ chế xếp hàng tác vụ (Task Queue) bằng Celery để xử lý hồ sơ không đồng bộ theo lô (batch processing), đồng thời tối ưu hóa kích thước prompt để kiểm soát lượng input token tối thiểu mà không ảnh hưởng tới chất lượng lập luận. |
